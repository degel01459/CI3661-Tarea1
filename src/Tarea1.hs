...
-- Nombre del estudiante: Kevin Briceño
-- Carnet: 15-11661
-------------------------------------------------------------------------------
...
esPalindromo s = pal s
  where
    pal []  = True
    pal [_] = True
    pal (x:xs) =
      let (initXs, lastX) = splitLast xs
      in if x /= lastX then False else pal initXs

    -- splitLast devuelve (init, last) para listas no vacías
    splitLast :: [a] -> ([a], a)
    splitLast [y]    = ([], y)
    splitLast (y:ys) = let (initYs, lastY) = splitLast ys
                       in (y:initYs, lastY)
    splitLast []     = error "splitLast: lista vacía"
...
productoParesRec [] = 1
productoParesRec (y:ys)
  | even y    = y * productoParesRec ys
  | otherwise = productoParesRec ys
...
parsearCondicional ss = map parseOne ss
  where
    parseOne :: String -> Either String Int
    parseOne s = case readMaybe s :: Maybe Int of
      Just n  -> Right n
